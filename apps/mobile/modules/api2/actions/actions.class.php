...
class api2Actions extends sfActions {
    ...
    // ex. http://www.getlokal.com/mobile_dev.php/api2/company?token=eea05e007a12c7b8dee730ab4de98eb2&id=14360&locale=bg
    // http://www.getlokal.com/mobile_dev.php/api2/company?token=eea05e007a12c7b8dee730ab4de98eb2&id=14360&locale=bg&lat=42.681173&long=23.310819
...
*/
    ...
    // ex. http://getlokal.com/mobile_dev.php/api2/page/slug/terms-of-use/locale/en
...
*/
    ...
    // ex. http://www.getlokal.com/mobile_dev.php/api2/event?token=eea05e007a12c7b8dee730ab4de98eb2&id=8285&locale=bg
    // http://www.getlokal.com/mobile_dev.php/api2/event?token=eea05e007a12c7b8dee730ab4de98eb2&id=8256&locale=bg&lat=42.681173&long=23.310819
	public function executeEvent(sfWebRequest $request) {
        ...
        if ($this->event->getFirstCompany()) {
            http://www.getlokal.com/mobile_dev.php/api2/company?token=eea05e007a12c7b8dee730ab4de98eb2&id=14360&locale=bg&lat=42.681173&long=23.310819
            $this->companyUrl = $this->generateUrl('default', array('module' => 'api2', 'action' => 'company', 'id' => $this->event->getFirstCompany()->getId(), 'locale' => $this->getUser()->getCulture()), true);
        }
        else {
            $this->companyUrl = '';
        }
		...
		for($i = $start; $i <= $end; $i ++) {
			if (isset ( $possitions [$i] )) {
				foreach ( $possitions [$i] as $news_item )
					if ($news_item)
						$sectors [$sector]['items'][]  = array ('identifier' => $news_item->getId (), 'type' => 2, 'internal' => 1, 'title' => $news_item->getTitle (), 'details1' => $news_item->getLine1 (), 'details2' => $news_item->getLine2 (), 'detauls3' => '', 'picture_url' => $news_item->getThumb ( 1 ), 'big_picture_url' => $news_item->getThumb (), 'content_url' => $news_item->getLink () );
			}
        ...
        */
        ...
        for($i = $start; $i <= $end; $i ++) {
            ...
            if (isset($events[$i])) { //foreach($events as $event)
                ...
                $returnItems[] = array('identifier' => $event->getId(), 'type' => 1, 'internal' => 0, 'title' => $event->getTitle(), 'share_url' => 'http://www.getlokal.' . $this->getUser()->getDomain() . '/' . $this->getUser()->getCulture() . '/d/event/show/id/' . $event->getId(), 'details1' => $date, 'details2' => $event->getPrice() ? $i18n->__('Ticket: ') . $event->getPriceValue() : '', 'details3' => $event->getFirstCompany() ? $event->getFirstCompany()->getTitle() . ' (' . number_format($event->distance, 2) . ' km)' : '', 'category' => $event->getCategory(), 'picture_url' => image_path($event->getThumb(), true), 'big_picture_url' => image_path($event->getThumb(0), true), 'content_url' => $this->generateUrl('default', array('module' => 'api2', 'action' => 'event', 'id' => $event->getId(), 'country' => $this->getUser()->getCountry()->getSlug(), 'locale' => $this->getUser()->getCulture()), true));
            }
		}
    ...
    // List of classifications - see search (explore)
	public function executeClassificationslist(sfWebRequest $request) {
        ...
        foreach ($sectors as $sector) {
            ...
            $return['classifications'][] = array('identifier' => $sector->getId(), 'title' => $sectorTitle, 'icon' => 'marker_' . $sector->getId());
        }
        */
    ...
    // test action
    // ex. http://www.getlokal.com/mobile_dev.php/api2/test
	...
	public function executeRegister(sfWebRequest $request) {
			...
			if (is_numeric ( $request->getParameter ( 'location' ) )) {
				...
				if ($request->getParameter ( 'location' ) != '') {
					...
					if ($city) {
                        ...
                        sfContext::getInstance()->getLogger()->emerg('GEOCODE: 2 WHERE: 2');
		...
		//
					...
					$url = "http://maps.googleapis.com/maps/api/geocode/json?latlng=" . str_replace ( '\'', '', $lat . ',' . $lng ) . "&sensor=false&language=en";
                    sfContext::getInstance()->getLogger()->emerg('GEOCODE: 2 WHERE: 3');
	...
	*/
	...
	public function executeFavoriteList(sfWebRequest $request) {
		...
		foreach ( $query->execute () as $company ) {
			$return [] = array ('identifier' => $company->getId (), 'title' => $company->getCompanyTitleByCulture(), 'ppp' => ($company->getActivePPPService ( true ) ? 1 : 0), 'phone' => $company->getPhone (), 'address' => $company->getDisplayAddress (), 'favourite' => 1, 'rating' => $company->getAverageRating (), 'reviews' => $company->getNumberOfReviews (), 'lat' => $company->getLocation ()->getLatitude (), 'long' => $company->getLocation ()->getLongitude (), 'distance' => $company->kms, 'icon' => 'marker_' . $company->getSectorId (), 'picture_url' => image_path ( $company->getThumb ( 0, true ), true ), 'share_url' => 'http://www.getlokal.' . $this->getUser ()->getDomain () . '/' . $this->getUser ()->getCulture () . '/' . $company->getCity ()->getSlug () . '/' . $company->getSlug (), 'content_url' => $this->generateUrl ( 'default', array ('module' => 'api2', 'action' => 'company', 'id' => $company->getId (), 'token' => $request->getParameter ( 'token' ), 'locale' => $this->getUser ()->getCulture () ), true ) );
	...
	public function executeSearchNear(sfWebRequest $request) {
		...
		if (is_numeric ( $where ) && ($where != - 1)) {
			...
			if ($city) {
                ...
                sfContext::getInstance()->getLogger()->emerg('GEOCODE: 2 WHERE: 4');
	...
	public function executeAddnewplace(sfWebRequest $request) {
		...
		if ($form->isValid ()) {
            ...
            sfContext::getInstance()->getLogger()->emerg('GEOCODE: 2 WHERE: 5');
			$string = file_get_contents ( $url ); // get json content
			$json_a = json_decode ( $string, true );
			$ch = curl_init ();
		...
		if (! $profile) {
			...
			if (! $user) {
				...
				if (isset ( $facebook_user_data ['location'] ['name'] ) && $facebook_user_data ['location'] ['name']) {
                    ...
                    sfContext::getInstance()->getLogger()->emerg('GEOCODE: 2 WHERE: 6');
