...
class api30Actions extends sfActions {
    private
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if (isset($events[$i])) { //foreach($events as $event)
                ...
                if (count($event->getEventPage())) {
                    ...
                    foreach ($eventPages as $eventPage) {
                        ...
                        $places[] = array(
                            ...
                            'content_url' => $this->generateUrl('default', array('module' => 'api30', 'action' => 'company', 'id' => $company->getId(), 'token' => $request->getParameter('token'), 'locale' => $culture), true),
                ...
                $returnItems[] = array(
                    ...
                    'content_url' => $this->generateUrl('default', array('module' => 'api30', 'action' => 'event', 'id' => $event->getId(), 'country' => $this->getUser()->getCountry()->getSlug(), 'locale' => $culture), true),
                    'places' => $places
                );
            }
        }
        ...
        if ($countResult) {
            ...
            foreach ($query as $c) {
                ...
                $items[] = array(
                    ...
                    'content_url' => $this->generateUrl('default', array('module' => 'api30', 'action' => 'company', 'id' => $c->getId(), 'token' => $request->getParameter('token'), 'locale' => $culture), true),
    ...
    public function executeAddnewplace(sfWebRequest $request) {
        ...
        $city_name = trim(substr($request->getParameter('location'), 0, strrpos($request->getParameter('location'), ',')));
            ...
            if(!strstr($request->getParameter('location'), '(null)')) {
                if ($form->isValid()) {
                    ...
                    if ($cntry) {
                        ...
                        ->andWhere('ct.name LIKE ? OR ct.name LIKE ?', array($geodata['city_en'], $geodata['city']))
                        ->fetchOne();
