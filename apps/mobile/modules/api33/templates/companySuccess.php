  ...
  <div class="company">
    <div class="image">
      ...
      <a href="getlokal://map" class="map">
        <img src="http<?php echo (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']) ? 's' : ''; ?>://maps.googleapis.com/maps/api/staticmap?center=<?php echo $company->getLocation()->getLatitude() ?>,<?php echo $company->getLocation()->getLongitude() ?>&zoom=15&size=400x400&maptype=roadmap&markers=icon:<?php echo image_path('gui/icons/marker_'. $company->getSectorId(), true) ?>%7Cshadow:false%7C<?php echo $company->getLocation()->getLatitude() ?>,<?php echo $company->getLocation()->getLongitude() ?>&sensor=false" class="map_image" />
      </a>
      <?php else: ?>
        <a href="getlokal://map">
          <img src="http<?php echo (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']) ? 's' : ''; ?>://maps.googleapis.com/maps/api/staticmap?center=<?php echo $company->getLocation()->getLatitude() ?>,<?php echo $company->getLocation()->getLongitude() ?>&zoom=15&size=600x200&maptype=roadmap&markers=icon:<?php echo image_path('gui/icons/marker_'. $company->getSectorId(), true) ?>%7Cshadow:false%7C<?php echo $company->getLocation()->getLatitude() ?>,<?php echo $company->getLocation()->getLongitude() ?>&sensor=false" class="map_image" />
        </a>
      <?php endif ?>
    </div>
    ...
    <?php endif ?>
      <?php file_put_contents('testtext', $company->getDisplayAddress());?>
