...
class devActions extends sfActions {
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if (isset($events[$i])) { //foreach($events as $event)
                ...
                if (count($event->getEventPage())) {
                    ...
                    foreach ($eventPages as $eventPage) {
                        ...
                        $places[] = array(
                            ...
                            'title' => $company->getCompanyTitle(null, null, true),
                            'ppp' => 0,
                            'favourite' => 0,
                            ...
                            'content_url' => $this->generateUrl('default', array('module' => 'dev', 'action' => 'company', 'id' => $company->getId(), 'token' => $request->getParameter('token'), 'locale' => $culture), true),
                ...
                $returnItems[] = array(
                    ...
                    'content_url' => $this->generateUrl('default', array('module' => 'dev', 'action' => 'event', 'id' => $event->getId(), 'country' => $this->getUser()->getCountry()->getSlug(), 'locale' => $culture), true),
                    'places' => $places
                );
            }
        }
    ...
    public function executeSearchFilter(sfWebRequest $request) {
        ...
        $results = array_merge(array(
            array(
                'identifier' => -1,
                'title' => $i18n->__('Getlokal recommended'),
    ...
    private function __searchBlank($request, $sLat, $sLng, $lat, $lng, $startIndex = 0, $endIndex = 14, $where, $offers = false) {
        ...
        $sql = "SELECT id, geodist({$latRad}, {$lngRad} , lat_rad, lng_rad) as distance,
                ...
                FROM search
                WHERE
                    proximity_reg <= 24 AND sector_id IN (" . implode(',', $this->visibleSectors) . ")
    ...
    private function __searchByKeyword($request, $sLat, $sLng, $lat, $lng, $startIndex, $endIndex, $keyword, $where) {
        ...
        if ($countResult) {
            ...
            foreach ($query as $c) {
                ...
                $items[] = array(
                    'identifier' => $c->getId(),
                    'title' => $c->getCompanyTitle(null, null, true),
                    ...
                    'content_url' => $this->generateUrl('default', array('module' => 'dev', 'action' => 'company', 'id' => $c->getId(), 'token' => $request->getParameter('token'), 'locale' => $culture), true),
        ...
        if ($vouchers) {
            foreach ($vouchers as $voucher) {
                ...
                $return['vouchers'][] = array(
                    ...
                    'offer' => $this->__formatOffer($offer, $voucher->kms)
                );
            }
        }
    ...
    public function executeAddnewplace(sfWebRequest $request) {
        ...
        if ($form->isValid()) {
            ...
            $company_location = CompanyLocationTable::geocode($address, $lat, $lng);
            if ($street_number) {
                $company_location->setStreetNumber($street_number);
            }

            $company->setCity($this->getUser()->getCity());
            $company->setCountry($this->getUser()->getCity()->getCountry());
        ...
        foreach ($query->execute() as $o) {
            ...
            if ($iCoId !== $o->getCompany()->getCountry()->getId()) {
                ...
                $cities["c" . $iCoId] = array(
                    'identifier' => "c" . $iCoId,
                    'title' => $o->getCompany()->getCountry()->getI18nName(),
                    'group' => 1,
                    'count' => 1
                );
            } else {
            ...
            if (array_key_exists($cId, $cities)) {
                ...
                $cities[$cId] = array(
                    'identifier' => $cId,
                    'title' => $o->getCompany()->getCity()->getI18nName(),
                    'group' => 0,
                    'count' => 1
                );
            }
    ...
    private function __formatOffer($o, $kms = null)
    {
        ...
        return array(
            ...
            'content_url' => $contentUrl,
            'share_url' => 'http://www.getlokal.' . $this->getUser()->getDomain() . '/' .
                $this->getUser()->getCulture() . '/d/offer/show/id/' . $o->getId(),
