...
class api22Actions extends sfActions {
    private
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if (isset($events[$i])) { //foreach($events as $event)
                ...
                if (count($event->getEventPage())) {
                    ...
                    foreach ($eventPages as $eventPage) {
                        ...
                        $places[] = array(
                            ...
                            'content_url' => $this->generateUrl('default', array('module' => 'api22', 'action' => 'company', 'id' => $company->getId(), 'token' => $request->getParameter('token'), 'locale' => $this->getUser()->getCulture()), true)
                        );
                    }
                }
                ...
                $returnItems[] = array(
                    ...
                    'share_url' => 'http' . ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']) ? 's' : '') . '://www.getlokal.' . $this->getUser()->getDomain($culture) . '/' . $culture . '/d/event/show/id/' . $event->getId(),
                    'details1' => $date,
                    ...
                    'content_url' => $this->generateUrl('default', array('module' => 'api22', 'action' => 'event', 'id' => $event->getId(), 'country' => $this->getUser()->getCountry()->getSlug(), 'locale' => $culture), true),
                    'places' => $places
                );
            }
        }
    ...
    //http://www.getlokal.com/mobile_dev.php/api22/getLocationAutocomplete?term=%D0%A1%D0%BE%D1%84%D0%B8%D1%8F
    public function executeGetLocationAutocomplete(sfWebRequest $request) {
        ...
        foreach ($this->rows as $row) {
            if ($this->in_array_r($row->getLocation(), $cities_names)) {
                if ($culture == 'en') {
                    ...
                    $result[] = array('identifier' => $row['id'], 'title' => $row->getLocation() . ', ' . mb_convert_case($row->getCounty()->getLocation(), MB_CASE_TITLE, 'UTF-8') . ', ' . mb_convert_case(call_user_func(array('Transliterate' . $partner_class, 'toLatin'), $row->getCounty()->getCountry()->getName()), MB_CASE_TITLE, 'UTF-8'), 'country' => 0);
                } else {
                    ...
                    $result[] = array('identifier' => $row['id'], 'title' => $row->getLocation() . ', ' . mb_convert_case($row->getCounty()->getLocation(), MB_CASE_TITLE, 'UTF-8') . ', ' . mb_convert_case($row->getCounty()->getCountry()->getName(), MB_CASE_TITLE, 'UTF-8'), 'country' => 0);
                }
            } else {
                if ($culture == 'en') {
    ...
    private function __getLocation($where) {
        ...
        // Use geocode
            ...
            sfContext::getInstance()->getLogger()->emerg('GEOCODE: 17');
    ...
    private function __searchByCS($request, $sLat, $sLng, $lat, $lng, $startIndex = 0, $endIndex = 14, $cs = null, $csType = 0) {
        $culture = $this->getUser()->getCulture();
        ...
        if (count($companiyIds)) {
            ...
            foreach ($companies as $company) {
                ...
                $items[] = array(
                    ...
                    'picture_url' => image_path($company->getThumb(0, true), true),
                    'content_url' => $this->generateUrl('default', array('module' => 'api22', 'action' => 'company', 'id' => $company->getId(), 'token' => $request->getParameter('token'), 'locale' => $culture), true),
                    'share_url' => 'http' . ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']) ? 's' : '') . '://www.getlokal.' . $this->getUser()->getDomain($culture) . '/' . $culture . '/' . $company->getCity()->getSlug() . '/' . $company->getSlug(),
                );
            }
        }
    ...
    private function __searchBlank($request, $sLat, $sLng, $lat, $lng, $startIndex = 0, $endIndex = 14) {
        $culture = $this->getUser()->getCulture();
        $con = mysqli_connect(sfConfig::get('app_search_host'), null, null, null, sfConfig::get('app_search_port'));
        mysqli_query($con, "SET NAMES utf8");
    ...
    private function __searchRecommended($request, $sLat, $sLng, $lat, $lng, $startIndex, $endIndex) {
        $con = mysqli_connect(sfConfig::get('app_search_host'), null, null, null, sfConfig::get('app_search_port'));
        mysqli_query($con, "SET NAMES utf8");
    ...
    private function __searchByKeyword($request, $sLat, $sLng, $lat, $lng, $startIndex, $endIndex, $keyword) {
        $culture = $this->getUser()->getCulture();
        $con = mysqli_connect(sfConfig::get('app_search_host'), null, null, null, sfConfig::get('app_search_port'));
        mysqli_query($con, "SET NAMES utf8");
        ...
        if ($countResult) {
            ...
            foreach ($query as $c) {
                ...
                $items[] = array(
                    ...
                    'content_url' => $this->generateUrl('default', array('module' => 'api22', 'action' => 'company', 'id' => $c->getId(), 'token' => $request->getParameter('token'), 'locale' => $culture), true),
                    'share_url' => 'http' . ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']) ? 's' : '') . '://www.getlokal.' . $this->getUser()->getDomain($culture) . '/' . $culture . '/' . $c->getCity()->getSlug() . '/' . $c->getSlug(),
                );
            }
        }
    ...
    public function executeSearchNear(sfWebRequest $request) {
        ...
        elseif ($cs) {
            // Recommended
            if ($cs == -1) {
                if ($result = $this->__searchRecommended($request, $sLat, $sLng, $lat, $lng, $startIndex, $endIndex)) {
                    return $this->renderText(json_encode(array_merge(array('status' => 'OK'), $result)));
                }
            }
            else {
                if ($result = $this->__searchByCS($request, $sLat, $sLng, $lat, $lng, $startIndex, $endIndex, $cs, $csType)) {
                    return $this->renderText(json_encode(array_merge(array('status' => 'OK'), $result)));
                }
            }
        }
    ...
    // test action
    // ex. http://www.getlokal.com/mobile_dev.php/api22/test
    ...
    public function executeAddnewplace(sfWebRequest $request) {
        ...
        $country_name = trim(substr(strrchr($request->getParameter('location'), ","), 1));
        $city_name = trim(strstr($request->getParameter('location'), ",", true));
                ...
                $url = CityTable::geocodeUrl(
                    ...
                    if (isset($geodata)) {
                        ...
                        $cntry = Doctrine::getTable('Country')
                            ->createQuery('cou')
                            ->where('cou.name_en LIKE ?', $geodata['country_en'])
                            ->andWhere('cou.slug != ?', '')
                            ->fetchOne();
                    }
                    if ($cntry) {
                        ...
                        ->where('ct.name LIKE ?', $geodata['city_en'])
                        ->fetchOne();
                    
                        if(!($cty)) {
                            $locationConcat = $geodata['city_en'].', '.$geodata['country_en'];
                            $url = CityTable::geocodeUrl(
                                $locationConcat,
                                null,
                                null,
        ...
        if (!$profile) {
            ...
            if (!$user) {
                ...
                if (isset($facebook_user_data ['location'] ['name']) && $facebook_user_data ['location'] ['name']) {
                    ...
                    sfContext::getInstance()->getLogger()->emerg('GEOCODE: 18');
