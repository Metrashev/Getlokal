...
?>
<ul class="nav nav-tabs default-form-tabs ul-tabs-company-settings" id="myTab" style="margin-bottom: 15px;">
<?php  foreach($menu[$active_key]['sub'] as $link => $name) {
		   $link_params = strpos($link, '?'); ?>
        	<li <?=($link == $key ? 'class="active"': '')?>><a href="<?php echo url_for($link.($link_params ? '&slug='. $company->getSlug() : '?slug='. $company->getSlug())) ?>" onclick="companySettingFormSubmition(href);return false;" ><?php echo $name;?></a></li>
<?php  } ?> 
</ul>