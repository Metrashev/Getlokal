...
<?php else:?>
<ul class="event_pictures" id="photos_tab">
	<?php 
		foreach ($images as $image) {
	?>
		<li class="upload-photo-event-details">
			<a href='<?= image_path($image->getThumb('preview'))?>' id="image-<?= $image->getId() ?>">
			<?php
		      	if ($image->getType()=='poster' ) {
		      		echo image_tag($image->getThumb('preview'),array('size'=>'220x170', 'title'=>$image->getCaption() ));
		      	}
		      	else {
	        ...
	        ?>
             ...
             ?>
			 <div class="wrapper-upload-photo-event-details">uploaded by:<a class="photo-uploader-event-details" href="<?= $user_url;?>"><?= $image->getUserProfile(); ?></a></div>
...
<div class="row"><!-- Form File -->
	<div class="default-container default-form-wrapper col-sm-12">
		<form id="imgCompanyForm" class="default-form clearfix" action="<?php echo url_for('event/show?id='. $event->getId() ) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
			<div class="row">
                <div class="col-sm-12">
                    <h2 class="form-title">
			...
			<?php
				if( $form['file']->hasError() || $form['caption']->hasError() ) { ?>
					<div class="form-message error">
						<?php echo __($sf_user->getFlash('error'), null, 'form') ?>					
					</div>
			<?php } ?>
