...
<div class="row">
	<div class="col-sm-12">
		<h2 class="form-title">
			<?php echo __('Login'); ?>
			<button type="button" class="close" onclick="$('.login_form_wrap').hide()"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		</h2>
	</div>
</div>
<div class="row">
	...
	<div class="row">
		<div class="col-sm-12">
				...
				<?php /*
		                 ...
		                 */ ?>
		        <a href="<?php echo url_for('company/registerPost?with_form=1')?>" class="register_pos default-btn pull-right"><?php echo __('Sign Up', null,'user')?></a>
		</div>
	</div>
...
<?php if(!empty($with_form)):?>
	...
	$('.register_pos').click(function() {
	    ...
	    $.ajax({
	        url: this.href,
	        beforeSend: function() {
	          $('.login_form_wrap').html(LoaderHTML);
	        },
