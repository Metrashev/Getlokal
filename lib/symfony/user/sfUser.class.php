  ...
  public function initialize(sfEventDispatcher $dispatcher, sfStorage $storage, $options = array())
  {
   ...
   // $ip_address = '2.63.255.255'; //ip russia ip
    ...
    if($ip_address != '127.0.0.1' && $ip_address != ''){
        ...
        $integer_ip = ( 16777216 * $positions[0] )
                    + (    65536 * $positions[1] )
                    + (      256 * $positions[2] )
                    +              $positions[3];
        ...
        $num_results = count($country);
        
        if($num_results>0){
            $county_slug = $country[0]['country_slugs'];
            
            if(isset($county_slug) && $county_slug !='' && $county_slug !==NULL){
                foreach(sfConfig::get('app_domain_slugs') as $dom){
                    if(strstr($host, $dom)){
                        $domain = $dom;
                        $domain_languages = sfConfig::get('app_languages_'.sfConfig::get('app_domain_to_culture_'.strtoupper($domain)));
                        break;
                    }
                    elseif(strstr($host, 'com') || strstr($host, 'my')){
                        $domain = 'bg';
                        $domain_languages = sfConfig::get('app_languages_'.sfConfig::get('app_domain_to_culture_'.strtoupper($domain)));
                    }
                }
                $newCulture =sfConfig::get('app_domain_to_culture_'.$county_slug);
                if(in_array($newCulture, $domain_languages)){
