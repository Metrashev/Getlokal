...
abstract class sfDoctrineRecord extends Doctrine_Record
{
  ...
  public function getDeepCacheReferences($obj_names = array()){
  	$arr = array();
  	if(!is_array($obj_names))$obj_names = array($obj_names);
      ...
      if (in_array($verb = substr($method, 0, 3), array('set', 'get')))
      {
        ...
        else
        {
          ...
          if ($table->hasField($underScored) || $table->hasRelation($underScored))
          {
            ...
            $camelCase = in_array($name, $opt_arr) ? $opt_arr[$name] :$table->getFieldName(sfInflector::camelize($name));
//             echo"<div class='xdebug'>\"$name\"=>\"$camelCase\"</div>";
