...
class importLocationsRuTask extends sfBaseTask
{
	protected $_countryId = 290;
	
	protected $_localLaguage = "ru";
	/*
	...
	protected $_iCityLat = 6;
	protected $_iCityLang = 7;
	protected $_iCityExternalId = -1;
	protected $_iCityCountyExternalId = -1;
	protected $_iCityCountyName = 4;
	
	protected $_iCountyName = 4;
...
// 		$this->name             = 'Location';		
		...
		$this->settings['source']['cities'] = sfConfig::get('sf_web_dir') . "/sources/russia/location_structure-april2014.csv";
		$this->settings['source']['counties'] = sfConfig::get('sf_web_dir') . "/sources/russia/location_structure-april2014.csv";
	...
	protected function _handleCounties(){
		...
		foreach($this->_counties as $county){
			...
			$newName = $this->_cleanName($newName);
			$columns[3] = 'name';
		...
		if(sizeof($insertData)){
			...
			if(is_array($bulk)){
				foreach ($bulk as $query){
					$countyConnection->execute($query);
				}	
			}else{
	...
	protected function _handleCities(){
		...
		foreach($this->_cities as $city){
			...
			if($countyExternalId != ''){
				...
				$newCountyId = $this->_getCountyIdByName($countyName);
			}
			if(isset($citiesByCounty[$newCountyId][$newName])){
				$citiesByCounty[$newCountyId][$newName] = 1;
			}
			...
			$cityData['district'] = $city[2];
			
			$slugs[] = $newSlug;
			$key = $this->_iCityExternalId > -1 ? $newExternalId : $newSlug;
			
			if(in_array("$newName-$newCountyId", $cityCounty)){
				$key = array_search("$newName-$newCountyId", $cityCounty);
				$toProcess[] = $key;
				$cityCounty[] = "$newName-$newCountyId";
				$toProcess[] = sizeof($cityCounty)-1;
			}else{
				$cityCounty[] = "$newName-$newCountyId";
			}			
			$allDataSlug[] = $cityData;
			/*if( ( !isset($this->_currentCities['ByName'][$newName]) || ($currntCityCountyId != 'none' && $currntCityCountyId != $newCountyId) ) && (!isset($this->_currentCities['ByExtId'][$newExternalId]) || $newExternalId == '') ){
				...
				$nameCounty[] = "$newName - $newCountyId";
			}	*/
			
		}
		array_unique($toProcess);
		foreach ($allDataSlug as $key => $info){
			if(in_array($key, $toProcess)){
				$info['name'] .= " ({$info['district']})";
			}
			unset($info['district']);
			if( (!isset($this->_currentCities['ByExtId'][$newExternalId]) || $newExternalId == '') ){
				if( ( !isset($this->_currentCities['ByName'][$newName]) || ($currntCityCountyId != 'none' && $currntCityCountyId != $newCountyId) ) ){
					$insertData[] = $info;
				}
			}
			
			$allDataSlug[$info['slug']] = $info;
			$allDataExId[$info['external_id']] = $info;
		}
		if(sizeof($insertData)){
			$bulk = $this->_bulkInsert($insertData, $table, $columns);
	...
	protected function _getCountyIdByName($countyName){
		if(isset($this->_currentCounties['ByName'][$countyName])){
			return $this->_currentCounties['ByName'][$countyName];
		}else{
			$countyName = mb_convert_case($countyName, MB_CASE_TITLE, "UTF-8");
			if(isset($this->_currentCounties['ByName'][$countyName])){
				return $this->_currentCounties['ByName'][$countyName];
			}else{
				$countyName = mb_convert_case($countyName, MB_CASE_LOWER, "UTF-8"); 
				return $this->_currentCounties['ByName'][$countyName];
			}
		}
	}
	...
	protected function _getCitiesSlugs(){
		$this->_currentCitiesSlugs = array();
		$sql = "SELECT
					c.id,
					c.slug					
				FROM `city` c";
		...
		$currentCities->execute();
		while ($city = $currentCities->fetch()) {
			$this->_currentCitiesSlugs[$city['id']] = $city['slug'];
		}
		return $this->_currentCitiesSlugs;
	}
	
	protected function _createSlugs(){
		
		$prefix = "extid-ru-";
		echo 'CREATING EXTERNAL IDS FOR COUNTIES'.PHP_EOL;
		$sql = "SELECT
					c.id,
					c.slug,
					c.external_id					
				FROM county c
				WHERE c.country_id=".$this->_countryId." AND c.external_id=''";
		$counties = $this->_connection->prepare($sql);
		$counties->execute();
		$i = 0;
		while ($county = $counties->fetch()) {
			$countiesId[] = $county['id'];
			$i++;
		}
		if($i){
			array_unique($countiesId);
			$query = "UPDATE `county` SET `external_id`=CONCAT('$prefix', `slug`) WHERE id IN (".implode(',', $countiesId).") LIMIT $i";
			$countyConnection = Doctrine::getConnectionByTableName("county");
			$countyConnection->execute($query);
			echo $query;
		}
		echo 'CREATING EXTERNAL IDS FOR COUNTIES - DONE: '.$i.' counties have new external ids'.PHP_EOL;
		//////////////////////////////////////////////////////////////////////////////////////////
		echo 'CREATING EXTERNAL IDS FOR CITIES'.PHP_EOL;
		$sql = "SELECT
					c.id,
					c.slug,
					c.external_id,
				...
				JOIN county cn
				ON cn.id=c.county_id
				WHERE cn.country_id=".$this->_countryId." AND c.external_id=''";
		$cities = $this->_connection->prepare($sql);
		$cities->execute();
		$i = 0;
		while ($city = $cities->fetch()) {
			$countiesId[] = $city['county_id'];
			$i++;
		}
		if($i){
			array_unique($countiesId);
			$query = "UPDATE `city` SET `external_id`=CONCAT('$prefix', `slug`) WHERE county_id IN (".implode(',', $countiesId).") LIMIT $i";
			$cityConnection = Doctrine::getConnectionByTableName("city");
			$cityConnection->execute($query);
			echo $query;
		}
		echo 'CREATING EXTERNAL IDS FOR CITIES - DONE: '.$i.' cities have new external ids'.PHP_EOL;
		
		
	}
	
	protected function _cleanName($name){
		/*$arRemove = array(' поселок', ' станица', ' село', ' деревня', ' коттеджный', ' городского', ' типа' ,' садовое','  товарищество', ' хутор', ' пансионат',
						  'поселок ', 'станица ', 'село ', 'деревня ', 'коттеджный ', 'городского ', 'типа ' ,'садовое ','товарищество ', 'хутор ', 'пансионат ',
				);
		$name = str_ireplace('"', '', $name);
		$name = str_ireplace($arRemove, '', $name);*/
	...
	////////////////////////////////
	private function _generateExternalIDs(){
