 ...
 */
class AdminRememberMeFilter extends sfFilter
{
	/**
	...
	public function execute($filterChain) {
		$cookieName = sfConfig::get ( 'app_admins_remember_cookie_name', 'paRemember' );
		
		if ($this->isFirstCall ()) {
			 if ($this->context->getUser()->isAnonymous() && $cookie = $this->context->getRequest()->getCookie($cookieName)){
 
				$q = Doctrine_Core::getTable ( 'AdminRememberKey' )->createQuery ( 'r' )->innerJoin ( 'r.PageAdmin u' )->where ( 'r.remember_key = ?', $cookie );
				
				if ($q->count ()) {
					
					$this->context->getUser ()->signInPageAdmin ( $q->fetchOne ()->PageAdmin );
				}
			}
		}
		$filterChain->execute ();
