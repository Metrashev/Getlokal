...
CREATE DEFINER=`stoycho`@`95.111.50.161` PROCEDURE `sphinx_company_video`(start_id BIGINT, end_id BIGINT, lang VARCHAR(3))
BEGIN
select
        cast(concat('4', `im`.`id`) as unsigned) AS `id`,
		cast(concat('4', `im`.`id`) as unsigned) AS `att_id`,
        `C`.`id` AS `doc_id`,
    ...
    where
        `C`.`status` = 0
            ...
            and `im`.`type` = 'video' AND `im`.`id` >= start_id AND `im`.`id` <= end_id;
END