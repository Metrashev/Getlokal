...
CREATE DEFINER=`stoycho`@`95.111.50.161` PROCEDURE `sphinx_company_description`(start_id BIGINT, end_id BIGINT, lang VARCHAR(3))
BEGIN
select
        ...
        '' AS `videoText`,
        0 AS `videoCount`,
		CONCAT(COALESCE(`ct`.`description`, ''), ' ', COALESCE(`cten`.`description`, '')) AS `description`,
		1 AS `descriptionCount`,
