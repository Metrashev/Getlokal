...
CREATE DEFINER=`stoycho`@`95.111.50.161` PROCEDURE `sphinx_company_reviews`(start_id BIGINT, end_id BIGINT, lang VARCHAR(3))
BEGIN
select
        cast(concat('2', `TR`.`id`) as unsigned) AS `id`,
		cast(concat('2', `TR`.`id`) as unsigned) AS `att_id`,
        `C`.`id` AS `doc_id`,
    ...
    where
        `C`.`status` = 0
            ...
            AND `TR`.`id` >= start_id AND `TR`.`id` <= end_id;
END