...
CREATE DEFINER=`stoycho`@`95.111.50.161` PROCEDURE `sphinx_company_detailed_description`(start_id BIGINT, end_id BIGINT, lang VARCHAR(3))
BEGIN
select
        cast(concat('5', `C`.`id`) as unsigned) AS `id`,
        `C`.`id` AS `doc_id`,
		...
		CONCAT(COALESCE(`ct`.`content`, ''), ' ', COALESCE(`cten`.`content`, '')) AS `detail_description`,
		1 AS `detail_descriptionCount`,
        '' AS `classification`,
    ...
    from
        `company_translation` `ct`
        join `company` `C` ON `ct`.`id` = `C`.`id` AND `ct`.`lang` = lang
