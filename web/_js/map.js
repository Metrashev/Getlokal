...
var map = {
  ...
  loadMarkers: function(jsonString){
	  ...
	  for (i = 0; i < companyCount; i++) {
		 s = objects[i];
		 if(s.lat != 0 && s.lng != 0){
			 var icon = new google.maps.MarkerImage('/images/gui/icons/small_'+s.icon+'.png', null, null, null, new google.maps.Size(40,40));
...
//		    	selected_id = s.id;
		        ...
		        map.overlay.load($("#overlays____ #"+this.data.id).html());
  ...
  initAutocomplete: function() {
    $.widget( "custom.locationComplete", $.ui.autocomplete, {
        ...
        _renderItemData: function (ul, item) {     
          ...
          return $('<li class="locations"></li>')
            ...
            .append('<a><i class="fa fa-location-arrow"></i>' + html + "<span class='clear'></span></a>")
            .appendTo(ul);
    ...
    $.widget( "custom.termComplete", $.ui.autocomplete, {
        ...
        _renderItemData: function (ul, item) {
          ...
          return $("<li></li>")
            .data("item.autocomplete", item)
            .append("<a" + href +">"+ img +" <span class='title'>" + item.title + "</span>" + details + "<span class='clear'></span></a>")
            .appendTo(ul);
        }
    });
    ...
    $([{
      ...
      to: $('#search_header_what').parent().parent(),
      ...
      s.el.termComplete({
        ...
        search: function(){
        $('#ui-id-1').html('');
        $('#ui-id-1').css('width',$(window).width()-608)
                     .append('<div class="loading-data"><img src="/images/gui/ui-loader.gif"/></div>')
                     .show();
        },
    ...
    $([{
      ...
      to: $('#search_header_where').parent().parent()
    }]).each(function (i, s) {
      var cache = {};
  ...
  init: function(load) {
    ...
    $('#map_reload').click(function() {
      // map.address = 'true';
        ...
        }, false, map.blankMap, map.autoTrigger);
...
*/
    ...
    if (map.totalObjects > map.itemsPerPage) {
	  ...
	  if (map.currentPage < map.totalPages) {
	      paginationRightHTML = '<a href="javascript:;" id="next"><span>Next</span></a>';
	  }

	  if (map.currentPage > 1) {
	      paginationLeftHTML = '<a href="javascript:;" id="prev"><span>Previous</span></a>';
	  }
	  if (map.totalPages != 1) {
...
$('.pager').remove();
      container.find('.pagerCenter').html(paginationHTML);
      // container.find('.pagerLeft').html(paginationLeftHTML);
      //  container.find('.pagerRight').html(paginationRightHTML);


      container.find('.pagerRight a, .pagerLeft a').click('live', function(){
          if ($(this).attr('id') == 'prev') {
            map.currentPage = parseInt(container.find('.pagerCenter a.current').attr('id').replace('page-', '')) - 1;
          } else {
            map.currentPage = parseInt(container.find('.pagerCenter a.current').attr('id').replace('page-', '')) + 1;
          }
          ...
          $('.pageCenter a#page-' + map.currentPage).addClass('current');
          
          map.offset = (map.currentPage - 1) * map.itemsPerPage;

          $('.listing_place_wrap').html('<div class="listing_place">loading...<img src="/images/gui/loading.gif" class="loading_icon" /></div>');
          //$('html,body').scrollTop($('.listing_tabs_wrap').offset().top - 60);
          $('html,body').scrollTop($('.listing_tabs_wrap').offset().top - 184);
      ...
      container.find('.pagerCenter a').click('live', function(){
          ...
          map.currentPage = parseInt(container.find('.pagerCenter a.current').attr('id').replace('page-', ''));
          map.offset = (map.currentPage - 1) * map.itemsPerPage;
        ...
        $(data).each(function(i,s) {
			if(s.latitude > 0 && s.longitude > 0){ 
		           if (s.is_ppp) {    
		        	...
		        	//  map.markers[s.id].icon = '/images/gui/icons/gray_small_'+s.icon+'.png';
		                  var iconPP = new google.maps.MarkerImage( '/images/gui/icons/gray_small_'+s.icon+'.png', null, null, null, new google.maps.Size(40,40));
		                  map.markers[s.id] = new google.maps.Marker({
		                  title: s.title,
		                  position: new google.maps.LatLng(s.latitude, s.longitude),
		                  map: map.map,
		        	  ...
		        	  map.markers[s.id].setZIndex(google.maps.Marker.MAX_ZINDEX + 1);
		          }
...
$(document).ready(function() {
    $("#add-place-btn-map").tooltip({
        placement: 'right',
        trigger: 'hover',
    });
    $('#search-btn-map').tooltip({
        placement: 'right',
        trigger: 'hover',
    });
});